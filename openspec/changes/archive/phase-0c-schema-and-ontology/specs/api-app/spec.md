...
(Previously: forbade `torch`, `transformers`, `sentence-transformers`, `langchain`, and `langchain-openai`. 0c clarifies that `langchain-core` is permitted as a transitive of `langgraph` while top-level `langchain` and `langchain-openai` remain forbidden.)
